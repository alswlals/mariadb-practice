...
-- 문제3.
-- 여직원과 남직원은 각 각 몇 명이나 있나요? (각각 출력|쿼리 두 개) if문 사용하기
select count(gender)
	from employees
    group by gender;
...
select count(distinct(emp_no)) as '근무하고 있는 직원의 수'
	from salaries;

-- 문제5.
-- 부서는 총 몇 개가 있나요?  count(distinct())
select count(distinct(dept_name)) as '부서'
	from departments;

-- 문제6.
-- 현재 부서 매니저는 몇 명이나 있나요?(역임 매너저는 제외) to_date
...
-- 문제11
-- 사번이 13250(Zeydy)인 지원이 직책 변경 상황을 시간순으로 출력해보세요. order by
