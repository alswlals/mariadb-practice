...
-- now(), sysdate() | now는 엔터 친 순간을 출력하는데(쿼리가 시작된 시간) sysdate는 출력되는 시간을 출력(쿼리가 실행된 시간)
...
-- 예) 각 사원들의 근속년수가 5년이 되는 날에 휴가를 보내준다면 각 사원의 5년 근속 휴가 날짜는 ?
...
-- type
-- 믄자 : varchar, char, text, CLOB(character Large OBject)
