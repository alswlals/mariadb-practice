-- 1) 집계 쿼리 : select 절에 통계함수(avg, max, min, count, sum, stddev, varance, ... )
...
-- 5) group by에 참여하고 있는 컬럼은 projection이 가능하다. : select절에 올 수 있다(예외)
-- 예제 : 각 사원별로 평균 연봉 출력
select emp_no, avg(salary)
	from salaries
    group by emp_no;
...
select emp_no, avg(salary) -- as avg_salary
	from salaries
    group by emp_no
    having avg(salary) >= 60000 -- having avg_salary >= 600000 도 가능
...
select emp_no, avg(salary) -- as avg_salary
    ...
    group by emp_no
    having avg(salary) >= 60000 -- having avg_salary >= 600000 도 가능
    order by avg(salary) asc; -- 출력하기 전 sorting 
